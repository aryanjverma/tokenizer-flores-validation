...
Throughout Studies~B and~C we write \texttt{gigatoken}(BPE) for the BPE arm trained with Gigatoken~\citep{roed2026gigatoken} and \texttt{supergigatoken}(SuperBPE) for the SuperBPE arm trained with our SuperBPE-capable Gigatoken fork~\citep{verma2026supergigatoken}.
...
\paragraph{Training.}
We train matched \texttt{gigatoken}(BPE) and \texttt{supergigatoken}(SuperBPE) tokenizers~\citep{roed2026gigatoken,verma2026supergigatoken} on an equal-byte \emph{scale} corpus: $160$\,MB per language, $960$\,MB total, drawn from FineWeb (English) and FineWeb-2 (other languages).
...
Pair verification confirms that \texttt{supergigatoken}(SuperBPE) carries the \texttt{gigatoken}(BPE) merge prefix through the transition point.
...
\begin{table}[t]
\centering
\caption{Study~B: English-relative token premium after equal-byte training.
Bold marks the better (lower) premium. SuperBPE worsens premium for Hungarian and Mandarin.}
...
\midrule
English           & 1.000 & 1.000 & --- \\
Hungarian         & \textbf{1.084} & 1.160 & $+0.076$ \\
Mandarin          & \textbf{0.912} & 1.082 & $+0.170$ \\
Hindi             & 1.259 & \textbf{1.256} & $-0.003$ \\
Swahili           & \textbf{1.032} & 1.056 & $+0.024$ \\
Haitian Creole    & 1.034 & \textbf{1.004} & $-0.030$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Study~B: fertility (tokens / whitespace word). Bold marks the better (lower) fertility.
Mandarin is reported but not interpreted as tokens/word.}
...
\midrule
English        & 1.321 & \textbf{1.124} \\
Hungarian      & 1.676 & \textbf{1.526} \\
Mandarin       & \textbf{12.759} & 12.874 \\
Hindi          & 1.420 & \textbf{1.206} \\
Swahili        & 1.402 & \textbf{1.221} \\
Haitian Creole & 1.293 & \textbf{1.068} \\
\bottomrule
\end{tabular}
\end{table}
...
Our default trainer for the SuperBPE arm (\texttt{supergigatoken}(SuperBPE)~\citep{verma2026supergigatoken}, built on Gigatoken~\citep{roed2026gigatoken}) implements SuperBPE stage~2 without the official \texttt{STAGE2\_REGEX}, bounding units by separator, newline, and a maximum unit length of 128.
Relative to the official SuperBPE trainer~\citep{liu2025superbpe}, this yields a Hindi-specific compression advantage for \texttt{supergigatoken}(SuperBPE) at small vocabularies (premium deltas of $-3.13\%$ at 4k and $-6.61\%$ at 16k in pilot cross-checks).
The \texttt{gigatoken}(BPE) arm remains comparable across trainers.
Hindi SuperBPE premiums in Table~\ref{tab:plan-a-premium} should therefore be read as \texttt{supergigatoken}(SuperBPE) numbers, not as a direct replication of published Liu et~al.\ SuperBPE.
...
Study~C asks the same question under matched multilingual pretraining with the Study~B \texttt{gigatoken}(BPE) and \texttt{supergigatoken}(SuperBPE) tokenizers~\citep{roed2026gigatoken,verma2026supergigatoken}.
We execute a cheap Phase~0 pilot first, then specify a Phase~1 $\sim$1B UniMax run whose result tables remain empty.

\subsection{Experiment process}

Both phases share the Study~B tokenizers (\texttt{gigatoken}(BPE) and \texttt{supergigatoken}(SuperBPE), vocabulary size $100{,}000$, SuperBPE transition at $80{,}000$) and the same six FineWeb~/FineWeb-2 languages.
Phase~0 materializes equal-\emph{token} packed shards, trains matched \texttt{olmo2\_100M} models, evaluates held-out cross-entropy as bits/token, then fertility-normalizes with Study~B FLORES UTF-8 bytes/token.
...
\end{equation}
where bytes/token is measured by encoding FLORES-200 \texttt{devtest} with the same \texttt{gigatoken}(BPE) / \texttt{supergigatoken}(SuperBPE) tokenizers (total UTF-8 bytes over $\CTC$).
...
(\texttt{gigatoken}(BPE) / \texttt{supergigatoken}(SuperBPE)).
Bold marks the better (lower) approximate BPB. Negative $\Delta$ favors SuperBPE.
...
\midrule
English        & \textbf{1.219} & 1.252 & $+0.032$ \\
Haitian Creole & 1.419 & \textbf{1.409} & $-0.010$ \\
Hindi          & \textbf{0.461} & 0.483 & $+0.022$ \\
Hungarian      & \textbf{1.055} & 1.121 & $+0.066$ \\
Swahili        & \textbf{1.147} & 1.161 & $+0.014$ \\
Mandarin       & \textbf{1.425} & 1.428 & $+0.003$ \\
\midrule
Micro (non-CJK)& \textbf{1.060} & 1.085 & $+0.025$ \\
\bottomrule
\end{tabular}
\end{table}
...
\texttt{supergigatoken}(SuperBPE)'s Hindi stage-2 deviation (\S\ref{sec:hindi-caveat}) means Hindi SuperBPE premiums are not directly comparable to published Liu et~al.\ numbers.
...
FLORES-200~\citep{goyal2022flores,nllb2022}, FineWeb~/FineWeb-2~\citep{penedo2024fineweb,penedo2025fineweb2}, SuperBPE~\citep{liu2025superbpe}, \texttt{gigatoken}(BPE)~\citep{roed2026gigatoken}, \texttt{supergigatoken}(SuperBPE)~\citep{verma2026supergigatoken}, and the OLMo stack~\citep{olmo2025} made these measurements possible.
